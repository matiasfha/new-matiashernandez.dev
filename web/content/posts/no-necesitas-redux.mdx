...
---
Una pregunta que me he encontrado varias veces de parte de quienes están aprendiendo React es si deben o no aprender Redux o si deben o no utilizar Redux.
...
* Combinó el modelo de [Flux](https://facebook.github.io/flux/) con el concepto de [Reducer](https://redux.js.org/glossary#reducer) creando un patrón de diseño sencillo que inmediatamente generó tracción.
* Y ofreció una solución para el manejo de estado global de una aplicación. Sin esto la forma en que podías resolver el problema de estado global (estado que puede ser utilizado por todos los componentes) era tener un estado (un objeto) definido en el componente root (normalmente llamado \`< App />\`) y pasar los valores de este estado a travez de props a los componentes hijos: una pesadilla.
...
Para ser claros, Redux es una gran herramienta, una solución inteligente a un problema complejo y es bueno usarlo pero cuando su uso es adecuado, cuando realmente tienes un estado global, pero si tienes estados simples, como un formulario o si una ventana modal debe o no mostrarse Redux es “overkill”.
